...
public class NoteLockSubsystem extends SubsystemBase {
   ...
   * m_encoder =
   * m_lockMotor.getAbsoluteEncoder(com.revrobotics.SparkAbsoluteEncoder.Type.
   * kDutyCycle);
   * currentRotationSetpoint = 0;
   * 
   * // m_encoder.setPositionConversionFactor(positionConverstionFactor);
   * // Might be required to initialize the encoder if it is in a known location
   * on
   * // startup.
   * 
   ...
   * // double ff = SmartDashboard.getNumber(smartDashboardPrefix +
   * "Feed Forward",
   * // 0);
